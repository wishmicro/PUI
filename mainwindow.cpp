...
void MainWindow::readIni()
{
   ...
   configRead->setIniCodec(QTextCodec::codecForName("utf-8"));
    ...
    {
        QMessageBox::information(this,"title","路径未获取");
    }
...
void MainWindow::addsignalWidget(WidgetInfo *info)
{
    ...
    else if(info->type.compare("MainWindow")==0){
...
{
    //    if(m_widgetarray==NULL){return;}
    //    WidgetInfo *poinerwidget=NULL;
    //    vecWidgetArray::iterator it;
    //    for(it=m_widgetarray->begin();it!=m_widgetarray->end();it++){
    //        //删除控件信息
    //        poinerwidget=*it;
    //        if(poinerwidget->objpointer){
    //            delete  poinerwidget->objpointer;
    //            poinerwidget->objpointer=NULL;
    //        }
